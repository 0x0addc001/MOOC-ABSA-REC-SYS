...
def main():
    ...
    class SentimentResult:
        ...
        def read_and_count_result(self, examples):
            ...
            self.aspect_frequency = aspect_frequency  # 每个方面出现的频率
            # {'酒店': 1, '手艺': 1, '服务': 4, '房间': 8, '设施': 1, '早餐': 1}
            self.opinion_frequency = opinion_frequency  # 每种观点词出现的频率
            # {'非常好': 1, '棒': 1, '好': 2, '干净': 4, '热情': 1, '大': 1, '小': 2}
            self.aspect_sentiment = aspect_sentiment  # 各方面的情感倾向统计
            # {'酒店': {'正向': 1}, '手艺': {'正向': 1}, '服务': {'正向': 3, '负向': 1}}
            self.aspect_opinion = aspect_opinion  # 各方面的观点词统计
            # {'酒店': {'非常好': 1}, '手艺': {'棒': 1}, '服务': {'好': 1, '热情': 1, '差': 1, '服务': 1}, '房间': {'干净': 4, '大': 1, '小': 2, '热': 1, '一般': 1, '漂亮': 1}}
            self.aspect_opinion_positives = aspect_opinion_positives  # 正向方面观点词统计
            # {'酒店非常好': 1, '手艺棒': 1, '服务好': 1, '房间干净': 4, '服务热 情': 1}
            self.aspect_opinion_negatives = aspect_opinion_negatives  # 负向方面观点词统计
            # {'房间小': 2, '房间热': 1, '服务差': 1, '房间一般': 1}
            self.descend_aspects = descend_aspects  # 将各方面出现次数降序排序
    ...
    # 数据库情感分析接口
    @app.post("/v1/dbEmotionAnalysis/", status_code=200)
    # 定义路径操作函数，当接口被访问将调用该函数
